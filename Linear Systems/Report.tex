...
Finally, I am now going to insert the $x_1$ component, the error, and the condition number for $n$ from 1 to 20 in the next page.
\begin{table*}[htb]
\centering % used for centering table
\begin{tabular}{c c c c} % centered columns (4 columns)
\hline %inserts double horizontal lines
n & $x_{1}$ & Error & $\kappa(A_n)$ \\ [0.5ex] % inserts table
%heading
...
It can be observed that as $n$ increases, the $x_i$ component gets incrementally closer to the actual $e-2$ value. The condition number also gets incrementally bigger. It can be noticed, however, that a large condition number doesn’t necessarily mean that the error will be large in all cases, just that it is possible to have a large error.
